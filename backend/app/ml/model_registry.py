...
class ModelRegistry:
    ...
    def _find_weights_file(self) -> Path | None:
    ...
    async def _load_demo_model(self) -> None:
        ...
        self.model_info = ModelInfo(
            ...
            metrics={"note": "Demo mode - scores are deterministic but not real predictions"},
        )
    ...
    async def _load_real_model(self, weights_path: Path) -> None:
        ...
        try:
            ...
            if state_dict:
                ...
                is_ensemble = any(
                    k.startswith(("efficientnet_b4.", "efficientnet_b5.", "xception."))
                    for k in sample_keys
                )
                logger.info(
                    f"Detected model type from state_dict: {'ensemble' if is_ensemble else 'single'}"
                )
