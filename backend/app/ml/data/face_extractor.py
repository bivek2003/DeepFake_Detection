...
class FaceExtractor:
    ...
    def __init__(
        ...
        if MTCNN_AVAILABLE:
            ...
            print(
                f"Using MTCNN face detector on {mtcnn_device} (forced for RTX 50 series compatibility)"
            )
        else:
    ...
    def _detect_mtcnn(self, rgb_image: np.ndarray) -> list[FaceDetection]:
        ...
        try:
            ...
            detections = []
            for i, (box, prob) in enumerate(zip(boxes, probs, strict=True)):
                ...
                detections.append(
                    FaceDetection(bbox=(x1, y1, x2, y2), confidence=float(prob), landmarks=landmark)
                )
    ...
    def _detect_opencv(self, image: np.ndarray) -> list[FaceDetection]:
        ...
        for x, y, w, h in faces:
            detections.append(
                FaceDetection(bbox=(x, y, x + w, y + h), confidence=1.0, landmarks=None)
            )
    ...
    def extract_from_video(
        ...
        if skip_existing:
            ...
            if len(existing) >= num_frames // 2:  # At least half extracted
                return {"video": str(video_path), "status": "skipped", "faces": len(existing)}

        # Open video
        cap = cv2.VideoCapture(str(video_path))
        if not cap.isOpened():
            return {"video": str(video_path), "status": "error", "error": "Could not open video"}

        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))

        if total_frames == 0:
            cap.release()
            return {"video": str(video_path), "status": "error", "error": "Video has no frames"}
        ...
        return {"video": str(video_path), "status": "success", "faces": extracted, "failed": failed}
    ...
    def process_dataset(
        ...
        for video_path in tqdm(videos, desc="Extracting faces"):
            ...
            result = self.extract_from_video(
                str(video_path), str(video_output_dir), num_frames=num_frames
            )
...
def main():
    ...
    extractor = FaceExtractor(
        output_size=args.size, device=args.device if torch.cuda.is_available() else "cpu"
    )
    ...
    if input_path.is_file():
        result = extractor.extract_from_video(
            str(input_path), args.output, num_frames=args.num_frames
        )
        print(f"Result: {result}")
    else:
        results = extractor.process_dataset(args.input, args.output, num_frames=args.num_frames)
        print("\nProcessing complete:")
